  ...
  */
  ...
  Future _parseAndDiscover(Path inputFile) {
    ...
    processHtmlFile(SourceFile file) {
      if (file == null || !_pathInfo.checkInputPath(file.path, messages: _messages)) return;
  ...
  Future<SourceFile> _parseHtmlFile(Path path) {
    return fileSystem.readTextOrBytes(path)
        .transform((source) {
          ...
          file.document = _time('Parsed', path, () => parseHtml(source, path, messages: _messages));
          return file;
        })
  ...
  void _addDartFile(SourceFile dartFile) {
    if (dartFile == null || !_pathInfo.checkInputPath(dartFile.path, messages: _messages)) return;
